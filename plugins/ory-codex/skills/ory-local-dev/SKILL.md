...
`@ory/codex` plugin ships two commands that
...
Both delegate to `npx -y -p @ory/codex ory-codex local <subcommand>`, so the same
...
`npx -y -p @ory/codex ory-codex local up --no-seed` instead.
...
```bash
npx -y -p @ory/codex ory-codex local configure
```
...
| Need                           | Command                                  |
|--------------------------------|------------------------------------------|
| Re-seed (after `local reset`)  | `npx -y -p @ory/codex ory-codex local seed`              |
| Tail Kratos logs               | `npx -y -p @ory/codex ory-codex local logs kratos -f`    |
| Check what's running           | `npx -y -p @ory/codex ory-codex local status`            |
| Print env vars to copy/paste   | `npx -y -p @ory/codex ory-codex local env`               |
...
`npx -y -p @ory/codex ory-codex local down`). Containers stop but volumes
...
```bash
npx -y -p @ory/codex ory-codex local reset
```
...
- **"Docker is not running"** — start Docker Desktop, then re-run
  `ory-local-up`.
- **Gateway never becomes healthy** — `npx -y -p @ory/codex ory-codex local logs
  gateway` and `npx -y -p @ory/codex ory-codex local logs kratos` to see which service
