...
```sh
npx -y -p @ory/codex ory-codex configure --project-url <URL> --oauth2-client-id <CLIENT_ID> [--api-key <KEY>]
```
...
(next step) or run `npx -y -p @ory/codex ory-codex permissions observe` first to flip back to
non-blocking.
...
Re-run `npx -y -p @ory/codex ory-codex permissions status` afterwards to confirm coverage.
...
```sh
npx -y -p @ory/codex ory-codex permissions observe
```
...
If a tool is unexpectedly blocked, run `npx -y -p @ory/codex ory-codex permissions status` to
...
| Command | Effect |
|---|---|
| `npx -y -p @ory/codex ory-codex permissions status` | Print mode + per-tool allow/deny breakdown for the current user. Read-only. |
| `npx -y -p @ory/codex ory-codex permissions bootstrap` | Write `use` tuples for the harness's built-in tools. Idempotent. |
| `npx -y -p @ory/codex ory-codex permissions bootstrap --dry-run` | Print what would be written without making any changes. |
| `npx -y -p @ory/codex ory-codex permissions observe` | Persist `permissionMode = "observe"` (denies log, allow through). |
| `npx -y -p @ory/codex ory-codex permissions enforce` | Persist `permissionMode = "enforce"` (denies block). |
