...
```bash
npx -y -p @ory/codex ory-codex dashboard
```
...
```bash
npx -y -p @ory/codex ory-codex dashboard --json
```

If the core services show as unreachable, start the local stack with
`ory-local-up` (or check state with `npx -y -p @ory/codex ory-codex local status`).
